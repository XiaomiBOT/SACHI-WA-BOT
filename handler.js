      ...
      for (let name in global.plugins) {
        ...
        if ((usedPrefix = (match[0] || '')[0])) {
          ...
          if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
            this.send2Button(m.chat, `Limit anda habis, silahkan beli melalui *${usedPrefix}buy*`, '© SACHI-WA-BOT', 'Buy', `${usedPrefix}buy`, 'Buy All', `${usedPrefix}buyall`, m)
...
*.enable delete*
`.trim(), '© SACHI-WA-BOT', 'SACHIBOT', '.1 delete', m.message)
  ...
  async onCall(json) {
    ...
    if (jid && isOffer) {
      ...
      m.reply('Please do not call bot!')
    }
  }
}
...
global.dfail = (type, m, conn) => {
  let msg = {
    rowner: 'Only _*OWNER*_ can use this command',
    owner: 'Only _*BOT OWNER*_ can use this command',
    mods: 'Only _*Moderator*_ can use this command',
    premium: 'This command for _*Premium*_ members only',
    group: 'This command can only be used in groups',
    private: 'This command can only be used in private chats',
      admin: 'This command is for the group *admin* only',
      botAdmin: 'Create bot as *group admin* to use this command',
